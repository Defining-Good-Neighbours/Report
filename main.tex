...
\begin{subfigure}[t]{0.35\textwidth}
    ...
    \caption{
    % Diagram showing the exudation of \ce{DMA}, shown in green, by the plant roof to improve Zinc, shown in red, solubility and cause it to move into the deficient area. This also shows the presence of microbes in the soil, shown in purple, which will consume the \ce{DMA}.
    }
    \label{fig:Zinc}
\end{subfigure}
% \hfill
...
\begin{subfigure}[t]{0.35\textwidth}
    ...
    \caption{
    % Diagram showing the movement of Zinc, in red, in response to DMA exudation, in green, and the consumption of \ce{DMA} by microbes, in purple.
    }
    \label{fig:my_label}
\end{subfigure}
...
Observe that \eqref{eq:var-time-model-1} and \eqref{eq:var-time-model-2} hold, by density, for any \(v \in H^1(\Omega)\). Notice that this assumes that \(\hat x\) and \(\hat y\), and their corresponding time derivatives, are defined pointwise in time. However, they no longer require to be defined pointwise for every \( (r,z) \in \Omega\).
...
\subsubsection{Original parameters}
 ...
 As we can see in this base model we observe a good amount of cooperation occurring between the two roots. The DMA exuded from the left has run into that on the right which is causing more $Zn$ to be made available to both roots. As the domain is symmetric and the roots close enough that their rhizospheres meet this is as expected. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\FloatBarrier
\subsubsection{Increased distance between roots}
 \begin{figure}
     ...
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/0.15ApartDMA24.png}
     \caption{0.15 apart \ce{DMA} after 24 hours}
     \end{subfigure}
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/0.15ApartZn24.png}
     \caption{0.15 apart \ce{Zn} after 24 hours}
     \end{subfigure}
 \end{figure}
 Now that the roots have been planted so that their rhizospheres are separated by a large amount of soil area we can observe they have a weaker effect upon each other with the $DMA$ graph in the left figure no longer showing any meeting of the exudations from both sides. The $Zn$ graph also reflect this, there is no longer a central band of accessible $Zn$ within the soil, instead we see two separate zones of increased $Zn$ concentration, Perhaps connected very weakly even at this distance as is shown in the very slight band of yellowing within the green area of the soil.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\FloatBarrier
\subsubsection{Increased maximum \ce{DMA} exudation}
 ...
 \begin{figure}
     ...
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/IncreasedBufferDMA24.png}
     \caption{0.08 apart, rate of \ce{DMA} exudation increased by $10^{-11}$, \ce{DMA} after 24 hours}
     \end{subfigure}
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/IncreasedBufferZn24.png}
     \caption{0.08 apart, rate of \ce{DMA} exudation increased by $10^{-11}$, \ce{Zn} after 24 hours}
     \end{subfigure}
 \end{figure}
 With a larger exudation of $DMA$ by the plant we see, as would be intuitively expected, a larger amount of $Zn$ in the soil. The bands we saw in the first plot are larger and more concentrated in these conditions. This suggets that rice plants with a stronger expression of the genes linked with $DMA$ exudation may be better at accessing $Zn$ in soil and leads us to question whether they would allow an increased distance between plants before this positive cooperation would fade, which we can investigate further.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\FloatBarrier
\subsubsection{Increased distance between plants and maximum \ce{DMA} exudation}
 ...
 \begin{figure}
     ...
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/IncreasedBufferAndDistancedDMA24.png}
     \caption{0.15 apart, rate of \ce{DMA} exudation increased by $10^{-11}$, \ce{DMA} after 24 hours}
    \end{subfigure}
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/IncreasedBufferAndDistanceZn24.png}
     \caption{0.15 apart, rate of \ce{DMA} exudation increased by $10^{-11}$, \ce{Zn} after 24 hours}
     \end{subfigure}
 \end{figure}
 With increased maximum $DMA$ exudation we observe the plants do indeed fare better when spread further apart, in comparison the the basic rice when moved apart this model shows more available $Zn$ to each plant. The cooperation still isnt as strong as the basic model when close together so its possible some sensitivity analysis investigating these two parameters would be an interesting thing to consider in future work to determine at which point increased $DMA$ stops allowing plant cooperation and at what distance this occurs. This would have to allow for the total amount of $Zn$ in the soil as a limiting factor of course, and keep in mind realistic limits on the amount of $DMA$ possible for rice plants to exude. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\FloatBarrier
 ...
 \begin{figure}
     ...
     \caption{Distance and \ce{DMA} exudation increased}
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/IncreasedZnAbsorbDMA24.png}
     \caption{0.08 apart, rate of \ce{Zn} absorption increased by $0.5e^{-2}$, \ce{DMA} after 24 hours}
     \end{subfigure}
     \centering
     \begin{subfigure}[t]{0.45\textwidth}
     \includegraphics[width=\textwidth,trim=100 100 150 150,clip]{Figures/testpics/IncreasedZnAbsorbZn24.png}
     \caption{0.08 apart, rate of \ce{Zn} absorption increased by $0.5e^{-2}$, Zn after 24 hours}
     \end{subfigure}
 \end{figure}
If instead we increase the rate of $Zn$ absorption by the rice plant we see that these plots remain very similar to our original model. However in this case the plant seems to begin to exude more $DMA$ earlier in its growth as we see higher $Zn$ concentration closer to the roots surface. \textcolor{red}{Check this later.}
...
\begin{align}
	\partial X_S &= \beta_1 X_L - \beta_2 X_S - \beta_3 X_S Y_{L,1} - \beta_4 X_S Y_{L,2}, \label{eq:ext_Xs} \\
...
\caption{$V_{max} = 2.5 \times 10^{-9}$ mol kg$^{-1}$ s$^{-1}$, i.e. citrate gets consumed by microbes. In (a) concentration of phosphate between the two roots after 24 hours; (b) comparison of phosphate absorbed by each of the roots with original (dashed line) and changed parameters (solid line).}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\FloatBarrier
\subsubsection{Increased phosphate buffer power}
Next, we look at the buffer power $b_X$, which is defined as the ratio of absorption and desorption of phosphate to and from soil particles. We consider a twofold increase in $b_X$ which can be thought of as $b_X * 2 = \beta_1 / (0.5 \beta_2)$, i.e. we reduce the desorption rate by a half. The implications of this change can be observed in Fig. \ref{fig:numexp_bxup}. The results are somewhat counter-intuitive at first glance as we would expect that the decreased desorption rate (or equivalently, an increased absorption rate) would cause less phosphate to be absorbed by the root as it would stick to the soil particles instead of into the soil solution. We can also explain this algebraically by taking \eqref{eq:ext_Xs} and expressing the ratio between phosphate concentration in soil solution and soil solid,
\begin{equation}
    \frac{X_L}{X_S} = \frac{1}{b_X} + \kappa_{X,1} Y_{L,1} + \kappa_{X,2} Y_{L,2},
\end{equation}
we see that with increased $b_X$, the concentration of $X$ in soil solid increases at the expense of $X_L$. However, we can try to justify the results by the fact that the exudates opeate on $P$ at soil particles, i.e. they solubilise $P$ they find at soil particles, and the more $P$ is found there (the higher the buffer power), the more of it gets solubilised into the solution and transported to the root surface.
% No dañes ve :v
% Buenas noches

% ya me voy, puedes enviar tranqui… oye por cierto, revisa el paquete .. . . . . . siunitx que cargué hace un siglo. Según veo están sufriendo con esas tablas habiendo comandos para las unidades~~~~ 
